...
switch lower(action)
    ...
    case 'shopos'
        ...
        alls=allchild(st.fig);
        h1 = uicontextmenu('Parent',st.fig);
        h12= alls(12);
        set(h12,'UIContextMenu',h1);
        uimenu(h1,'Separator','off','Label','Plot the Raw Time Series of current voxel',...
            'CallBack',{@plottimes_ccm2,XYZ,st},...
            'Interruptible','off','BusyAction','Cancel','Enable','on');
        h2 = uicontextmenu('Parent',st.fig);
        h13= alls(13);
        set(h13,'UIContextMenu',h2);
        uimenu(h2,'Separator','off','Label','Plot the Raw Time Series of current voxel',...
            'CallBack',{@plottimes_ccm2,XYZ,st},...
            'Interruptible','off','BusyAction','Cancel','Enable','on');
        h3 = uicontextmenu('Parent',st.fig);
        h14= alls(14);
        set(h14,'UIContextMenu',h3);
        uimenu(h3,'Separator','off','Label','Plot the Raw Time Series of current voxel',...
            'CallBack',{@plottimes_ccm2,XYZ,st},...
            'Interruptible','off','BusyAction','Cancel','Enable','on');
        %%=========changes by Changming Chen finished==================
...
% function plottimes_ccm(src,event,where,behinput)
% global st;
% handles=guidata(src);
% if strcmp(behinput,'cc')
%     y=spm_sample_vol(st.vols{1:end},XYZ(1),XYZ(2),XYZ(3),st.hld);
%     figure;
%     plot([1:length(st.vols)],y);
% elseif strcmp(behinput,'dd')
% end


function plottimes_ccm2(src,event,XYZ,st)
a=spm_vol(st.vols{1}.fname);
b=spm_read_vols(a);
numtrs=size(b,4);
for i=1:numtrs
    y(i,1)=b(round(XYZ(1)),round(XYZ(2)),round(XYZ(3)),i);
end
%     for i=1:numtrs
%         y(i,1)=spm_sample_vol(st.vols{i},XYZ(1),XYZ(2),XYZ(3),st.hld);
%     end
...
h=figure('tag','plot_ccm','Name','Plot Time Series of Designated Voxel, by Changming Chen');
ctc=[0 0 1];
ytc=[0 0 1];
xtc=[0 0 1];
...
p1=plot((1:numtrs),y);
box off;
title(gca,['[',sprintf('%d %d %d',[round(XYZ(1)),round(XYZ(2)),round(XYZ(3))]),']'],'tag','colortitle','Color',ctc);
set(gca,'YLim',[min(y)-0.1*range(y),max((y)+0.1*range(y))]);
h1=uicontextmenu;
set(p1, 'UIContextMenu', h1);
u1 = uimenu(h1,'Label','Load Onset File','Callback',{@plotonsets,y});
ylb=ylabel('MRI signal','FontWeight','bold','tag','yl_ccm','Color',ytc);
ylpos=get(ylb,'Position');    yticks=get(gca,'YTick'); ylpos(2)=max(yticks)-range(yticks)*0.15; set(ylb,'Position',ylpos);
xlb=xlabel('Time','FontWeight','bold','tag','xl_ccm','Color',xtc);
xlmt=get(gca,'XLim'); pos=get(xlb,'Position');    pos(1)=xlmt(2)-range(xlmt)*0.15;  set(xlb,'Position',pos);

function plotonsets(hObject,eventdata, y)
handles=guidata(hObject);
[fn,pn,~] =uigetfile('*.mat','Please select the onset (.Mat) file you want to import, one file only');
load(deblank([pn,fn]));
numtrs=length(y);
numcons=numel(names);
conds=zeros(numtrs,numcons);
a=jet(numcons);
yr=range(y);
temp=findall(gcf,'tag','xl_ccm');
for i=1:numcons
    for jj=1:numel(onsets{i})
        conds([round(onsets{i}(jj))+1:round(onsets{i}(jj)+durations{i}(jj))]',i)=1;
    end
    hold on;
    area(min(y)-0.1*yr+conds(:,i)*yr,'FaceColor',a(i,:),'FaceAlpha',0.08,'EdgeColor','none');
end
hold on;
lgd=legend([{'MRI signal'},names],'EdgeColor','none','Color','none','Position',[0.80 0.82 0.16 0.12]);
